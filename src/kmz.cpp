...
#define ZIP_RDONLY 0
	...
	if (zindex == -1) {
		...
		ret.kmz_status = KMZOpenStatus::KMZOpenStatus::NoDoc; /* TODO_2_LATER: in original viking code we didn't return a "no doc.kml" status here, even though there was a status value defined for this event. */
		goto kmz_cleanup;
	}
