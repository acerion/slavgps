...
bool DEM::intersect(const LatLonBBox & other_bbox) const
{
	...
	if (this->horiz_units == DEMHorizontalUnit::LatLonArcSeconds) {
		...
		const UTM dem_northeast_utm(this->max_north_seconds, this->max_east_seconds, this->utm.zone(), this->utm.band_letter());
		const UTM dem_southwest_utm(this->min_north_seconds, this->min_east_seconds, this->utm.zone(), this->utm.band_letter());
