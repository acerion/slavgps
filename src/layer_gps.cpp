...
static MeasurementScale<Duration> gpsd_retry_interval_scale(0, 10000, 10, 10, DurationType::Unit::E::Seconds, 0);
...
#define CURRENT_HAS_BETTER_FIX  (this->current_rt_data.fix.mode > MODE_2D && this->previous_rt_data.fix.mode <= MODE_2D)
	...
	const Altitude alt = Altitude(this->current_rt_data.fix.altitude, AltitudeType::Unit::E::Metres); /* Altitude::is_valid() may return false. */
	if (!create_this_tp && (cur_timestamp != last_timestamp)) {
		const Altitude last_alt = Altitude(this->previous_rt_data.fix.altitude, AltitudeType::Unit::E::Metres); /* Altitude::is_valid() may return false. */
	...
	if (create_this_tp) {
		...
		new_tp->course = Angle(this->current_rt_data.fix.track, AngleType::Unit::E::Degrees); /* From the lecture of code of gpsd package, it seems that 'track' is in degrees (e.g. "Course over ground, degrees from true north."). */
...
void LayerGPS::rt_cleanup_layer_children(void)
{
	...
	if (this->realtime_record && this->realtime_track) {
		if (this->realtime_track->empty()) {
			this->trw_children[GPS_CHILD_LAYER_TRW_REALTIME]->remove_child(this->realtime_track);
