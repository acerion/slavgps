...
static struct {
	const char * symbol_name;     /* Icon names used by gpsbabel, garmin. */
	const char * old_symbol_name; /* Keep backward compatible. */
	int num;
	...
	/* "symbol_name" are in 'Title Case' like in gpsbabel. This is needed for
	   ...
	   necessary to allow GarminSymbol::get_hashed_symbol_name() to match the lower case
...
static void init_icons()
{
	...
	for (int i = 0; i < G_N_ELEMENTS(garmin_syms); i++) {
		g_hash_table_insert(icons, (void *) garmin_syms[i].symbol_name, KINT_TO_POINTER (i));
		g_hash_table_insert(old_icons, (void *) garmin_syms[i].old_symbol_name, KINT_TO_POINTER (i));
	}
}
...
QPixmap * GarminSymbols::get_wp_symbol(const char * symbol_name)
{
	if (!symbol_name) {
		return NULL;
	}
	...
	void * gp;
	void * x;
	if (g_hash_table_lookup_extended(icons, symbol_name, &x, &gp)) {
		return get_wp_sym_from_index((int) ((long) gp));
	} else if (g_hash_table_lookup_extended(old_icons, symbol_name, &x, &gp)) {
		return get_wp_sym_from_index((int) ((long) gp));
	} else {
		return NULL;
	}
...
const char * GarminSymbols::get_hashed_symbol_name(const char * symbol_name)
{
	if (!symbol_name) {
		return NULL;
	}
	...
	void * x;
	if (g_hash_table_lookup_extended(icons, symbol_name, &x, &gp)) {
		return garmin_syms[(int) ((long) gp)].symbol_name;
	} else if (g_hash_table_lookup_extended(old_icons, symbol_name, &x, &gp)) {
		return garmin_syms[(int) ((long) gp)].symbol_name;
	} else {
		return NULL;
...
void GarminSymbols::populate_symbols_list(GtkListStore *list)
{
	for (int i = 0; i < G_N_ELEMENTS (garmin_syms); i++) {
