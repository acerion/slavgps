...
static void vik_gobject_builder_init(VikGobjectBuilder *object)
{
...
static void vik_gobject_builder_finalize(GObject *object)
{
...
static void vik_gobject_builder_new_object(VikGobjectBuilder *self, GObject *object)
{
...
static void vik_gobject_builder_class_init (VikGobjectBuilderClass *klass)
{
...
static void _end_element(GMarkupParseContext * context,
			 const char          * element_name,
			 void                * user_data,
			 GError             ** error)
{
		  ...
		  GError             ** error)
{
	if (strcmp (g_markup_parse_context_get_element (context), "property") == 0) {
		...
		if (gtype != 0 && property_name != NULL) {
			/* Parameter value. */
			/* We have to retrieve the expected type of the value in order to do the correct transformation. */
			GObjectClass *oclass;
