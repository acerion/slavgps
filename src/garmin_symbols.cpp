...
/* Containers used for normalization of symbol names.
   Symbol name (possibly lower- or mixed-case) -> index in
   garmin_symbols. We will get normalized symbol name from
   garmin_symbols using the index. */
...
static struct {
	const QString symbol_name;     /* Icon names used by gpsbabel, garmin. */
	const QString old_symbol_name; /* Keep backward compatible. */
	int num;
	const char * description;
	const char * icon_small;
	const char * icon_large;
	QPixmap * pixmap;
} garmin_symbols[] = {
	/* "symbol_name" are in 'Title Case' like in gpsbabel. This is needed for
	   ...
	   version with the identifier stored in "symbol_name".
	*/
	/*---------------------------------------------------------------
	  Marine symbols
	  ---------------------------------------------------------------*/
	...
	{ "",                                "",                 -1,      NULL,                                NULL,                 NULL,                       NULL } /* Guard. */
};




static void init_symbol_names()
{
	int i = 0;
	while (garmin_symbols[i].num != -1) {
		symbol_names[QString(garmin_symbols[i].symbol_name)] = i;
		old_symbol_names[QString(garmin_symbols[i].old_symbol_name)] = i;
		i++;
	}
}
...
{
	/* Ensure icon data exists to either directly load icon or scale from the other set. */
	if (!garmin_symbols[i].pixmap && (garmin_symbols[i].icon_small || garmin_symbols[i].icon_large)) {
		if (Preferences::get_use_large_waypoint_icons()) {
			if (garmin_symbols[i].icon_large) {
	...
	return garmin_symbols[i].pixmap;
}
...
QPixmap * GarminSymbols::get_wp_symbol(const QString & symbol_name)
{
	...
	auto iter = symbol_names.find(symbol_name);
	if (iter != symbol_names.end()) {
		return get_wp_sym_from_index(iter->second);
	}

	iter = old_symbol_names.find(symbol_name);
	if (iter != old_symbol_names.end()) {
		return get_wp_sym_from_index(iter->second);
	}
...
void GarminSymbols::clear_symbols(void)
{
	...
	int i = 0;
	while (garmin_symbols[i].num != -1) {
		if (garmin_symbols[i].pixmap) {
			delete garmin_symbols[i].pixmap;
			garmin_symbols[i].pixmap = NULL;
		}
		i++;
	}
}
