...
static void draw_scroll_cb(VikWindow * vw, GdkEventScroll * event);
...
{
	vw->window->statusbar_update(message, vs_type);
	return;
}
	...
	if (window_count < MAX_WINDOWS) {
		...
		if (a_vik_get_restore_window_state()) {
			...
			if (a_settings_get_boolean(VIK_SETTINGS_WIN_SIDEPANEL, &sidepanel)) {
				if (! sidepanel) {
					gtk_widget_hide(GTK_WIDGET(vw->window->layers_panel->gob));
					GtkWidget *check_box = gtk_ui_manager_get_widget(vw->window->uim, "/ui/MainMenu/View/SetShow/ViewSidePanel");
			...
			if (a_settings_get_boolean(VIK_SETTINGS_WIN_STATUSBAR, &statusbar)) {
				if (! statusbar) {
					gtk_widget_hide(GTK_WIDGET(vw->window->viking_vs));
					GtkWidget *check_box = gtk_ui_manager_get_widget(vw->window->uim, "/ui/MainMenu/View/SetShow/ViewStatusBar");
					gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(check_box), false);
				}
			}
			...
			if (a_settings_get_boolean(VIK_SETTINGS_WIN_TOOLBAR, &toolbar)) {
				if (! toolbar) {
					gtk_widget_hide(toolbar_get_widget(vw->window->viking_vtb));
					GtkWidget *check_box = gtk_ui_manager_get_widget(vw->window->uim, "/ui/MainMenu/View/SetShow/ViewToolBar");
					gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(check_box), false);
				}
			}
			...
			if (a_settings_get_boolean(VIK_SETTINGS_WIN_MENUBAR, &menubar)) {
				if (! menubar) {
					gtk_widget_hide(gtk_ui_manager_get_widget(vw->window->uim, "/ui/MainMenu"));
					GtkWidget *check_box = gtk_ui_manager_get_widget(vw->window->uim, "/ui/MainMenu/View/SetShow/ViewMainMenu");
					gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(check_box), false);
				}
			}
		}
...
static int determine_location_thread(VikWindow * vw, void * threaddata)
{
	struct LatLon ll;
	char * name = NULL;
	int ans = a_vik_goto_where_am_i(vw->window->viewport, &ll, &name);

	int result = a_background_thread_progress(threaddata, 1.0);
	if (result != 0) {
		vw->window->statusbar_update(_("Location lookup aborted"), VIK_STATUSBAR_INFO);
	...
	if (ans) {
		...
		vw->window->viewport->set_zoom(zoom);
		vw->window->viewport->set_center_latlon(&ll, false);

		char *message = g_strdup_printf(_("Location found: %s"), name);
		vw->window->statusbar_update(message, VIK_STATUSBAR_INFO);
		free(name);
		free(message);
...
void vik_window_new_window_finish(VikWindow * vw)
{
	...
	if (vw->window->filename) {
		return;
	}

	if (a_vik_get_startup_method() == VIK_STARTUP_METHOD_SPECIFIED_FILE) {
		vw->window->open_file(a_vik_get_startup_file(), true);
		if (vw->window->filename) {
			return;
		}
	}
	...
	if(a_vik_get_add_default_map_layer()) {
		LayerMaps * layer = new LayerMaps(vw->window->viewport);
		layer->rename(_("Default Map"));

		vw->window->layers_panel->get_top_layer()->add_layer(layer, true);

		vw->window->draw_update();
	}

	// If not loaded any file, maybe try the location lookup
	if (vw->window->loaded_type == LOAD_TYPE_READ_FAILURE) {
		if (a_vik_get_startup_method() == VIK_STARTUP_METHOD_AUTO_LOCATION) {

			vik_statusbar_set_message(vw->window->viking_vs, VIK_STATUSBAR_INFO, _("Trying to determine location..."));
...
static void open_window(VikWindow * vw, GSList *files)
{
	...
	while (cur_file) {
		...
		if (vw->window->filename && check_file_magic_vik(file_name)) {
			VikWindow *newvw = vik_window_new_window();
			if (newvw) {
				newvw->window->open_file(file_name, true);
			}
		} else {
			vw->window->open_file(file_name, change_fn);
		}
		free(file_name);
...
static void zoom_changed(GtkMenuShell * menushell, void * user_data)
{
	...
	double current_zoom = vw->window->viewport->get_zoom();
	if (current_zoom != 0.0 && zoom_request != current_zoom) {
		vw->window->viewport->set_zoom(zoom_request);
...
static void vik_window_init(VikWindow * vw)
{
	...
	// Own signals
	g_signal_connect_swapped(G_OBJECT(vw->window->viewport->vvp), "updated_center", G_CALLBACK(center_changed_cb), vw);
	// Signals from GTK
	g_signal_connect_swapped(G_OBJECT(vw->window->viewport->vvp), "expose_event", G_CALLBACK(draw_sync_cb), vw->window);
	g_signal_connect_swapped(G_OBJECT(vw->window->viewport->vvp), "configure_event", G_CALLBACK(window_configure_event), vw);
	...
	g_signal_connect_swapped(G_OBJECT(vw->window->viewport->vvp), "scroll_event", G_CALLBACK(draw_scroll_cb), vw);
	int a = g_signal_connect_swapped(G_OBJECT(vw->window->viewport->vvp), "button_press_event", G_CALLBACK(draw_click), vw);
	g_signal_connect_swapped(G_OBJECT(vw->window->viewport->vvp), "button_release_event", G_CALLBACK(draw_release), vw);
	g_signal_connect_swapped(G_OBJECT(vw->window->viewport->vvp), "motion_notify_event", G_CALLBACK(draw_mouse_motion), vw);

	g_signal_connect_swapped(G_OBJECT(vw->window->layers_panel->gob), "update", G_CALLBACK(draw_update_cb), vw->window);
	g_signal_connect_swapped(G_OBJECT(vw->window->layers_panel->gob), "delete_layer", G_CALLBACK(vik_window_clear_highlight_cb), vw);
	...
	gtk_paned_pack1(GTK_PANED(vw->hpaned), GTK_WIDGET (vw->window->layers_panel->gob), false, true);
	gtk_paned_pack2(GTK_PANED(vw->hpaned), GTK_WIDGET (vw->window->viewport->vvp), true, true);

	/* This packs the button into the window (a gtk container). */
	gtk_box_pack_start(GTK_BOX(vw->main_vbox), vw->hpaned, true, true, 0);

	gtk_box_pack_end(GTK_BOX(vw->main_vbox), GTK_WIDGET(vw->window->viking_vs), false, true, 0);
...
static bool key_press_event(VikWindow * vw, GdkEventKey *event, void * data)
{
	...
	// Standard Ctrl+KP+ / Ctrl+KP- to zoom in/out respectively
		...
		vw->window->viewport->zoom_in();
		vw->window->draw_update();
		return true; // handled keypress
	}
	else if (event->keyval == GDK_KEY_KP_Subtract && (event->state & modifiers) == GDK_CONTROL_MASK) {
		vw->window->viewport->zoom_out();
		vw->window->draw_update();
		return true; // handled keypress
	}

	if (map_download) {
		vw->window->simple_map_update(map_download_only_new);
		return true; // handled keypress
	}

	Layer * layer = vw->window->layers_panel->get_selected();
	if (layer && vw->vt->active_tool != -1 && vw->vt->tools[vw->vt->active_tool].ti.key_press) {
		int ltype = vw->vt->tools[vw->vt->active_tool].layer_type;
		if (layer && ltype == layer->type) {
			return vw->vt->tools[vw->vt->active_tool].ti.key_press(layer, event, vw->vt->tools[vw->vt->active_tool].state);
		}
	}
	...
	if (event->keyval == GDK_Escape) {
		...
		if (check_box) {
			...
			if (!state) {
				gtk_widget_show(gtk_ui_manager_get_widget(vw->window->uim, "/ui/MainMenu"));
				gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(check_box), true);
				return true; /* handled keypress */
			}
		}
...
static bool delete_event(VikWindow * vw)
{
	...
	if (window_count == 1) {
		...
		if (a_vik_get_restore_window_state()) {
			...
			a_settings_set_boolean(VIK_SETTINGS_WIN_SIDEPANEL, GTK_WIDGET_VISIBLE (GTK_WIDGET(vw->window->layers_panel->gob)));

			a_settings_set_boolean(VIK_SETTINGS_WIN_STATUSBAR, GTK_WIDGET_VISIBLE (GTK_WIDGET(vw->window->viking_vs)));

			a_settings_set_boolean(VIK_SETTINGS_WIN_TOOLBAR, GTK_WIDGET_VISIBLE (toolbar_get_widget(vw->window->viking_vtb)));
		...
		a_settings_set_integer(VIK_SETTINGS_WIN_SAVE_IMAGE_WIDTH, vw->window->draw_image_width);
		a_settings_set_integer(VIK_SETTINGS_WIN_SAVE_IMAGE_HEIGHT, vw->window->draw_image_height);
		a_settings_set_boolean(VIK_SETTINGS_WIN_SAVE_IMAGE_PNG, vw->window->draw_image_save_as_png);
...
{
	VikWindow * vw = (VikWindow *) this->vw;

	vw->window->viewport->sync();
	this->draw_status();
}
...
static void draw_status_tool(VikWindow * vw)
{
	if (vw->current_tool == TOOL_LAYER) {
		...
		vik_statusbar_set_message(vw->window->viking_vs, VIK_STATUSBAR_TOOL, vik_layer_get_interface((VikLayerTypeEnum) vw->tool_layer_id)->tools[vw->tool_tool_id].radioActionEntry.tooltip);
	} else {
		vik_statusbar_set_message(vw->window->viking_vs, VIK_STATUSBAR_TOOL, _(tool_names[vw->current_tool]));
	}
}
...
void Window::draw_status()
{
	...
	double xmpp = vw->window->viewport->get_xmpp();
	double ympp = vw->window->viewport->get_ympp();
	char *unit = vw->window->viewport->get_coord_mode() == VIK_COORD_UTM ? _("mpp") : _("pixelfact");
	if (xmpp != ympp) {
...
static void window_configure_event(VikWindow * vw)
{
	...
	if (first) {
		...
		gdk_window_set_cursor(gtk_widget_get_window(GTK_WIDGET(vw->window->viewport->vvp)), vw->viewport_cursor);
	}
}
...
void Window::draw_redraw()
{
	...
	this->trigger = NULL;
	VikLayer *old_trigger = (VikLayer *) vw->window->viewport->get_trigger();
	...
	if (! new_trigger) {
		...
		vw->window->viewport->set_trigger(new_trigger); /* todo: set to half_drawn mode if new trigger is above old */
	} else {
		vw->window->viewport->set_half_drawn(true);
	}
	...
	if (this->viewport->get_draw_highlight()) {
		if (this->containing_trw && (this->selected_tracks || this->selected_waypoints)) {
			this->containing_trw->draw_highlight_items(this->selected_tracks, this->selected_waypoints, vw->window->viewport);

		} else if (this->containing_trw && (this->selected_track || this->selected_waypoint)) {
			this->containing_trw->draw_highlight_item((Track *) this->selected_track, this->selected_waypoint, vw->window->viewport);

		} else if (this->selected_trw) {
			this->selected_trw->draw_highlight(vw->window->viewport);
		}
	}
	...
	vw->window->viewport->set_half_drawn(false); /* just in case. */
}
...
static void draw_click(VikWindow * vw, GdkEventButton *event)
{
	gtk_widget_grab_focus(GTK_WIDGET(vw->window->viewport->vvp));
...
{
	if (vw->pan_x != -1) {
		vw->window->viewport->set_center_screen(vw->window->viewport->get_width()/2 - event->x + vw->pan_x,
						       vw->window->viewport->get_height()/2 - event->y + vw->pan_y);
		vw->pan_move = true;
		vw->pan_x = event->x;
		vw->pan_y = event->y;
		vw->window->draw_update();
...
{
	if (vw->window->viewport->get_drawmode() == VIK_VIEWPORT_DRAWMODE_UTM) {
...
#define BUFFER_SIZE 50
	...
	free(lon);
	lon = NULL;
	vik_statusbar_set_message(vw->window->viking_vs, VIK_STATUSBAR_POSITION, pointer_buf);
...
static bool vik_window_pan_timeout(VikWindow * vw)
{
	...
	vw->pan_move = false;
	vw->single_click_pending = false;
	vw->window->viewport->set_center_screen(vw->delayed_pan_x, vw->delayed_pan_y);
	vw->window->draw_update();
...
static void vik_window_pan_release(VikWindow * vw, GdkEventButton *event)
{
	...
	if (vw->pan_move == false) {
		...
		if (vw->single_click_pending) {
			...
			vw->window->viewport->set_center_screen(vw->pan_x, vw->pan_y);
		}
	} else {
		vw->window->viewport->set_center_screen(vw->window->viewport->get_width()/2 - event->x + vw->pan_x,
						vw->window->viewport->get_height()/2 - event->y + vw->pan_y);
...
static VikLayerToolFuncStatus ruler_click(Layer * layer, GdkEventButton *event, ruler_tool_state_t *s)
{
	...
	if (event->button == 1) {
		...
		vik_statusbar_set_message(s->vw->window->viking_vs, VIK_STATUSBAR_INFO, temp);
		free(temp);

		s->oldcoord = coord;
	} else {
		s->viewport->set_center_screen((int) event->x, (int) event->y);
		s->vw->window->draw_update();
	}
...
static VikLayerToolFuncStatus ruler_move(Layer * layer, GdkEventMotion *event, ruler_tool_state_t *s)
{
	...
	if (s->has_oldcoord) {
		...
		vik_statusbar_set_message(vw->window->viking_vs, VIK_STATUSBAR_INFO, temp);
		free(temp);
	}
	return VIK_LAYER_TOOL_ACK;
}
...
static void zoomtool_resize_pixmap(zoom_tool_state_t *zts)
{
	...
	w1 = zts->vw->window->viewport->get_width();
	h1 = zts->vw->window->viewport->get_height();

	if (!zts->pixmap) {
		// Totally new
		zts->pixmap = gdk_pixmap_new(gtk_widget_get_window(GTK_WIDGET(zts->vw->window->viewport->vvp)), w1, h1, -1);
	}
...
{
	zts->vw->window->modified = true;
	unsigned int modifiers = event->state & (GDK_SHIFT_MASK | GDK_CONTROL_MASK);

	VikCoord coord;
	int x, y;
	int center_x = zts->vw->window->viewport->get_width() / 2;
	int center_y = zts->vw->window->viewport->get_height() / 2;
	...
	if (modifiers == (GDK_CONTROL_MASK | GDK_SHIFT_MASK)) {
		// This zoom is on the center position
		zts->vw->window->viewport->set_center_screen(center_x, center_y);
		if (event->button == 1) {
			zts->vw->window->viewport->zoom_in();
		} else if (event->button == 3) {
			zts->vw->window->viewport->zoom_out();
		}
	} else if (modifiers == GDK_CONTROL_MASK) {
		// This zoom is to recenter on the mouse position
		zts->vw->window->viewport->set_center_screen((int) event->x, (int) event->y);
		if (event->button == 1) {
			zts->vw->window->viewport->zoom_in();
		} else if (event->button == 3) {
			zts->vw->window->viewport->zoom_out();
		...
		/* make sure mouse is still over the same point on the map when we zoom */
		zts->vw->window->viewport->screen_to_coord(event->x, event->y, &coord);
		if (event->button == 1) {
			zts->vw->window->viewport->zoom_in();
		} else if (event->button == 3) {
			zts->vw->window->viewport->zoom_out();
		}
		zts->vw->window->viewport->coord_to_screen(&coord, &x, &y);
		zts->vw->window->viewport->set_center_screen(center_x + (x - event->x),
						     center_y + (y - event->y));
	}

	if (!skip_update) {
		zts->vw->window->draw_update();
	}
...
static VikLayerToolFuncStatus zoomtool_move(Layer * layer, GdkEventMotion *event, zoom_tool_state_t *zts)
{
	...
	if (zts->bounds_active && modifiers == GDK_SHIFT_MASK) {
		...
		gdk_draw_drawable(zts->pixmap,
				  gtk_widget_get_style(GTK_WIDGET(zts->vw->window->viewport->vvp))->black_gc,
				  zts->vw->window->viewport->get_pixmap(),
				  0, 0, 0, 0, -1, -1);
		...
		// Draw the box
		gdk_draw_rectangle(zts->pixmap, gtk_widget_get_style(GTK_WIDGET(zts->vw->window->viewport->vvp))->black_gc, false, xx, yy, width, height);

		// Only actually draw when there's time to do so
		if (draw_buf_done) {
			static void * pass_along[3];
			pass_along[0] = gtk_widget_get_window(GTK_WIDGET(zts->vw->window->viewport->vvp));
			pass_along[1] = gtk_widget_get_style(GTK_WIDGET(zts->vw->window->viewport->vvp))->black_gc;
...
static VikLayerToolFuncStatus zoomtool_release(Layer * layer, GdkEventButton *event, zoom_tool_state_t *zts)
{
	...
	if (zts->bounds_active && modifiers == GDK_SHIFT_MASK &&
		...
		zts->vw->window->viewport->screen_to_coord(zts->start_x, zts->start_y, &coord1);
		zts->vw->window->viewport->screen_to_coord(event->x, event->y, &coord2);
		...
		VikCoord new_center;
		vik_coord_load_from_latlon(&new_center, zts->vw->window->viewport->get_coord_mode(), &average);
		zts->vw->window->viewport->set_center_coord(&new_center, false);
		...
		if (modifiers == GDK_SHIFT_MASK) {
			...
			zts->vw->window->viewport->set_center_screen(event->x, event->y);
			if (event->button == 1) {
				zts->vw->window->viewport->zoom_in();
				zts->vw->window->viewport->zoom_in();
				zts->vw->window->viewport->zoom_in();
...
static VikLayerToolFuncStatus pantool_click(Layer * layer, GdkEventButton * event, VikWindow * vw)
{
	vw->window->modified = true;
...
static VikLayerToolFuncStatus pantool_move(Layer * layer, GdkEventMotion * event, VikWindow * vw)
{
	vik_window_pan_move(vw, event);
	return VIK_LAYER_TOOL_ACK;
}

static VikLayerToolFuncStatus pantool_release(Layer * layer, GdkEventButton * event, VikWindow * vw)
{
	if (event->button == 1) {
		vik_window_pan_release(vw, event);
	}
...
static VikLayerToolFuncStatus selecttool_click(Layer * layer, GdkEventButton * event, tool_ed_t * t)
{
	...
	if (event->button == 1) {
		...
		if (event->state & VIK_MOVE_MODIFIER) {
			...
			std::list<Layer *> * layers = t->vw->window->layers_panel->get_all_layers_of_type(VIK_LAYER_TRW, false); // Don't get invisible layers
			clicker ck;
			ck.cont = true;
			ck.viewport = t->vw->window->viewport;
			ck.event = event;
			ck.tool_edit = t;
			...
			if (ck.cont) {
				GtkTreeIter iter;
				VikTreeview *vtv = t->vw->window->layers_panel->get_treeview();
		...
		if (layer->visible) {
			...
			if (t->vw->window->selected_track || t->vw->window->selected_waypoint) {
				layer->show_selected_viewport_menu(event, t->vw->window->viewport);
			}
		}
	}
...
static void draw_pan_cb(GtkAction * a, VikWindow * vw)
{
	...
	Layer * sel = vw->window->layers_panel->get_selected();
	if (sel && sel->vt->tree->get_editing()) {
		return;
	}

	if (!strcmp(gtk_action_get_name(a), "PanNorth")) {
		vw->window->viewport->set_center_screen(vw->window->viewport->get_width()/2, 0);
	} else if (!strcmp(gtk_action_get_name(a), "PanEast")) {
		vw->window->viewport->set_center_screen(vw->window->viewport->get_width(), vw->window->viewport->get_height()/2);
	} else if (!strcmp(gtk_action_get_name(a), "PanSouth")) {
		vw->window->viewport->set_center_screen(vw->window->viewport->get_width()/2, vw->window->viewport->get_height());
	} else if (!strcmp(gtk_action_get_name(a), "PanWest")) {
		vw->window->viewport->set_center_screen(0, vw->window->viewport->get_height()/2);
	}
	vw->window->draw_update();
}
...
static void draw_zoom_cb(GtkAction * a, VikWindow * vw)
{
	...
	switch (what) {
	case -3: vw->window->viewport->zoom_in(); break;
	case -4: vw->window->viewport->zoom_out(); break;
	case -1: vw->window->viewport->set_zoom(0.5); break;
	case -2: vw->window->viewport->set_zoom(0.25); break;
	default: vw->window->viewport->set_zoom(what);
	}
	vw->window->draw_update();
}
...
static void draw_goto_cb(GtkAction * a, VikWindow * vw)
{
	...
	if (!strcmp(gtk_action_get_name(a), "GotoLL")) {
		...
		vik_coord_to_latlon(vw->window->viewport->get_center(), &llold);
		if (a_dialog_goto_latlon(GTK_WINDOW(vw), &ll, &llold)) {
			vik_coord_load_from_latlon(&new_center, vw->window->viewport->get_coord_mode(), &ll);
		} else {
			return;
		}
	} else if (!strcmp(gtk_action_get_name(a), "GotoUTM")) {
		struct UTM utm, utmold;
		vik_coord_to_utm(vw->window->viewport->get_center(), &utmold);
		if (a_dialog_goto_utm(GTK_WINDOW(vw), &utm, &utmold)) {
			vik_coord_load_from_utm(&new_center, vw->window->viewport->get_coord_mode(), &utm);
		} else {
			return;
		}
	} else {
	...
	vw->window->viewport->set_center_coord(&new_center, true);
	vw->window->draw_update();
}
...
static void center_changed_cb(VikWindow * vw)
{
	...
	*/
	...
	if (action_forward) {
		gtk_action_set_sensitive(action_forward, vw->window->viewport->forward_available());
	}

	toolbar_action_set_sensitive(vw->window->viking_vtb, "GoForward", vw->window->viewport->forward_available());
}
...
static void draw_goto_back_and_forth(GtkAction * a, VikWindow * vw)
{
	...
	if (!strcmp(gtk_action_get_name(a), "GoBack")) {
		changed = vw->window->viewport->go_back();
	} else if (!strcmp(gtk_action_get_name(a), "GoForward")) {
		changed = vw->window->viewport->go_forward();
	} else {
		return;
	}
...
{
	// Only get 'new' maps
	vw->window->simple_map_update(true);
}
...
static void menu_addlayer_cb(GtkAction * a, VikWindow * vw)
{
	...
	for (type = 0; ((VikLayerTypeEnum) type) < VIK_LAYER_NUM_TYPES; type++) {
		if (!strcmp(vik_layer_get_interface((VikLayerTypeEnum) type)->name, gtk_action_get_name(a))) {
			if (vw->window->layers_panel->new_layer((VikLayerTypeEnum) type)) {
				vw->window->draw_update();
				vw->window->modified = true;
			}
		}
...
{
	vw->window->layers_panel->cut_selected();
	vw->window->modified = true;
}

static void menu_paste_layer_cb(GtkAction * a, VikWindow * vw)
{
	if (vw->window->layers_panel->paste_selected()) {
		vw->window->modified = true;
	}
}
...
static void menu_properties_cb(GtkAction * a, VikWindow * vw)
{
	if (! vw->window->layers_panel->properties()) {
...
GtkWidget *get_show_widget_by_name(VikWindow * vw, char const * name)
{
	...
	GtkWidget *widget = gtk_ui_manager_get_widget(vw->window->uim, path);
	free(path);
...
{
	bool next_state = !vw->window->show_side_panel;
	...
	if(next_state != menu_state) {
		...
		vw->window->toggle_side_panel();
	}
}

static void tb_full_screen_cb(GtkAction * a, VikWindow * vw)
{
	bool next_state = !vw->window->show_full_screen;
	GtkWidget *check_box = get_show_widget_by_name(vw, gtk_action_get_name(a));
	bool menu_state = gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(check_box));
	if (next_state != menu_state) {
		gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(check_box), next_state);
	} else {
		vw->window->toggle_full_screen();
	}
}

static void tb_view_statusbar_cb(GtkAction * a, VikWindow * vw)
{
	bool next_state = !vw->window->show_statusbar;
	GtkWidget *check_box = get_show_widget_by_name(vw, gtk_action_get_name(a));
	bool menu_state = gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(check_box));
	if (next_state != menu_state) {
		gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(check_box), next_state);
	} else {
		vw->window->toggle_statusbar();
	}
}

static void tb_view_toolbar_cb(GtkAction * a, VikWindow * vw)
{
	bool next_state = !vw->window->show_toolbar;
	GtkWidget *check_box = get_show_widget_by_name(vw, gtk_action_get_name(a));
	bool menu_state = gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(check_box));
	if (next_state != menu_state) {
		gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(check_box), next_state);
	} else {
		vw->window->toggle_toolbar();
	}
}

static void tb_view_main_menu_cb(GtkAction * a, VikWindow * vw)
{
	bool next_state = !vw->window->show_main_menu;
	GtkWidget *check_box = get_show_widget_by_name(vw, gtk_action_get_name(a));
	bool menu_state = gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(check_box));
	if (next_state != menu_state) {
		gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(check_box), next_state);
	} else {
		vw->window->toggle_main_menu();
	}
}

static void tb_set_draw_scale_cb(GtkAction * a, VikWindow * vw)
{
	bool next_state = !vw->window->viewport->get_draw_scale();
	GtkWidget *check_box = get_show_widget_by_name(vw, gtk_action_get_name(a));
	bool menu_state = gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(check_box));
	if (next_state != menu_state) {
		gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(check_box), next_state);
	} else {
		vw->window->viewport->set_draw_scale(next_state);
		vw->window->draw_update();
	}
}

static void tb_set_draw_centermark_cb(GtkAction * a, VikWindow * vw)
{
	bool next_state = !vw->window->viewport->get_draw_centermark();
	GtkWidget *check_box = get_show_widget_by_name(vw, gtk_action_get_name(a));
	bool menu_state = gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(check_box));
	if (next_state != menu_state) {
		gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(check_box), next_state);
	} else {
		vw->window->viewport->set_draw_centermark(next_state);
		vw->window->draw_update();
	}
}

static void tb_set_draw_highlight_cb(GtkAction * a, VikWindow * vw)
{
	bool next_state = !vw->window->viewport->get_draw_highlight();
	GtkWidget *check_box = get_show_widget_by_name(vw, gtk_action_get_name(a));
	bool menu_state = gtk_check_menu_item_get_active(GTK_CHECK_MENU_ITEM(check_box));
	if (next_state != menu_state) {
		gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(check_box), next_state);
	} else {
		vw->window->viewport->set_draw_highlight(next_state);
		vw->window->draw_update();
	}
}
...
{
	vw->window->viewport->show_centers(GTK_WINDOW(vw));
}

static void menu_delete_layer_cb(GtkAction * a, VikWindow * vw)
{
	if (vw->window->layers_panel->get_selected()) {
		vw->window->layers_panel->delete_selected();
		vw->window->modified = true;
	} else {
...
{
	VikWindow * vw = (VikWindow *) window->vw;

	bool next_state = !vw->window->show_full_screen;
	GtkToggleToolButton *tbutton =(GtkToggleToolButton *)toolbar_get_widget_by_name(vw->window->viking_vtb, gtk_action_get_name(a));
	if (tbutton) {
...
static void toolbox_add_tool(toolbox_tools_t *vt, VikToolInterface *vti, int layer_type)
{
	...
	if (vti->create) {
		vt->tools[vt->n_tools].state = vti->create(vt->vw, vt->vw->window->viewport);
	} else {
...
static void toolbox_activate(toolbox_tools_t *vt, char const *tool_name)
{
	...
	Layer * layer = vt->vw->window->layers_panel->get_selected();
#if 0
	if (!layer) {
		return;
	}
	...
	if (vt->active_tool != -1 && vt->tools[vt->active_tool].ti.move) {
		...
		if (ltype == TOOL_LAYER_TYPE_NONE || (layer && ltype == layer->type))
			if (VIK_LAYER_TOOL_ACK_GRAB_FOCUS == vt->tools[vt->active_tool].ti.move(layer, event, vt->tools[vt->active_tool].state))
				gtk_widget_grab_focus(GTK_WIDGET(vt->vw->window->viewport->vvp));
	}
}
...
{
	gtk_action_activate(gtk_action_group_get_action(((VikWindow *) this->vw)->action_group, vik_layer_get_interface((VikLayerTypeEnum) layer_id)->tools[tool_id].radioActionEntry.name));
}
...
{
	GtkToggleToolButton *tbutton = (GtkToggleToolButton *)toolbar_get_widget_by_name(vw->window->viking_vtb, name);
	if (tbutton) {
...
static void menu_cb(GtkAction *old, GtkAction * a, VikWindow * vw)
{
	...
	if (gtk_widget_get_window(GTK_WIDGET(vw->window->viewport->vvp)))
		/* We set cursor, even if it is NULL: it resets to default */
		gdk_window_set_cursor(gtk_widget_get_window(GTK_WIDGET(vw->window->viewport->vvp)), vw->viewport_cursor);
...
{
	return ((VikWindow *) this->vw)->pan_move;
}
...
static void on_activate_recent_item(GtkRecentChooser *chooser, Window * window)
{
	...
	if (filename != NULL) {
		...
		if (vw->window->filename) {
			...
			g_signal_emit(G_OBJECT(vw), window_signals[VW_OPENWINDOW_SIGNAL], 0, filenames);
			// NB: GSList & contents are freed by main.open_window
		} else {
...
void Window::setup_recent_files()
{
	...
	menu_item = gtk_ui_manager_get_widget(this->uim, "/ui/MainMenu/File/OpenRecentFile");
...
void Window::set_busy_cursor()
{
	...
	gdk_window_set_cursor(gtk_widget_get_window(GTK_WIDGET(vw->window->viewport->vvp)), vw->busy_cursor);
...
{
	vw->window->open_file(filename, change_filename);
}
...
void Window::open_file(char const * filename, bool change_filename)
{
	...
	LayerAggregate * agg = vw->window->layers_panel->get_top_layer();
	this->loaded_type = a_file_load(agg, vw->window->viewport, filename);
	...
	case LOAD_TYPE_VIK_SUCCESS:
		{
			...
			/* Update UI */
			if (change_filename) {
				vw->window->set_filename(filename);
			}
			mode_button = vw->window->get_drawmode_button(vw->window->viewport->get_drawmode());
			vw->window->only_updating_coord_mode_ui = true; /* if we don't set this, it will change the coord to UTM if we click Lat/Lon. I don't know why. */
			gtk_check_menu_item_set_active(GTK_CHECK_MENU_ITEM(mode_button), true);
			vw->window->only_updating_coord_mode_ui = false;

			vw->window->layers_panel->change_coord_mode(vw->window->viewport->get_coord_mode());
			...
			bool vp_state_scale = vw->window->viewport->get_draw_scale();
			...
			if (vp_state_scale != ui_state_scale) {
				vw->window->viewport->set_draw_scale(!vp_state_scale);
				vw->window->toggle_draw_scale(NULL);
			}
			bool vp_state_centermark = vw->window->viewport->get_draw_centermark();
			...
			if (vp_state_centermark != ui_state_centermark) {
				vw->window->viewport->set_draw_centermark(!vp_state_centermark);
				vw->window->toggle_draw_centermark(NULL);
			}
			bool vp_state_highlight = vw->window->viewport->get_draw_highlight();
			...
			if (vp_state_highlight != ui_state_highlight) {
				vw->window->viewport->set_draw_highlight(!vp_state_highlight);
				vw->window->toggle_draw_highlight(NULL);
			}
		}
	...
	default:
		...
		vw->window->update_recently_used_document(filename);
		vw->window->draw_update();
		break;
	}
	...
	if (! success || restore_original_filename) {
		...
		vw->window->set_filename(original_filename);
	}
...
#ifdef VIKING_PROMPT_IF_MODIFIED
		if ((vw->window->modified || vw->window->filename) && newwindow) {
#else
		if (vw->window->filename && newwindow) {
#endif
			...
			while (cur_file) {
				...
				if (newwindow && check_file_magic_vik(file_name)) {
					...
					if (first_vik_file) {
						vw->window->open_file(file_name, true);
...
static bool save_file_as(GtkAction * a, VikWindow * vw)
{
	...
	char* auto_save_name = g_strdup(vw->window->get_filename());
	...
	while (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
		...
		if (g_file_test(fn, G_FILE_TEST_EXISTS) == false || a_dialog_yes_or_no(GTK_WINDOW(dialog), _("The file \"%s\" exists, do you wish to overwrite it?"), a_file_basename(fn))) {
			vw->window->set_filename(fn);
			rv = vw->window->window_save();
			if (rv) {
				vw->window->modified = false;
				free(last_folder_files_uri);
				last_folder_files_uri = gtk_file_chooser_get_current_folder_uri(GTK_FILE_CHOOSER(dialog));
...
void Window::export_to_common(VikFileType_t vft, char const * extension)
{
	...
	if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
		...
		gtk_widget_destroy(dialog);
		if (dir) {
			if (!vw->window->export_to(layers, vft, dir, extension)) {
...
static void export_to_gpx_cb(GtkAction * a, VikWindow * vw)
{
	vw->window->export_to_common(FILE_TYPE_GPX, ".gpx");
}

static void export_to_kml_cb(GtkAction * a, VikWindow * vw)
{
	vw->window->export_to_common(FILE_TYPE_KML, ".kml");
}
...
static void file_properties_cb(GtkAction * a, VikWindow * vw)
{
	char *message = NULL;
	if (vw->window->filename) {
		if (g_file_test(vw->window->filename, G_FILE_TEST_EXISTS)) {
			// Get some timestamp information of the file
			GStatBuf stat_buf;
			if (g_stat(vw->window->filename, &stat_buf) == 0) {
				char time_buf[64];
...
#endif
				message = g_strdup_printf("%s\n\n%s\n\n%s", vw->window->filename, time_buf, size);
				free(size);
			}
		} else {
...
static void menu_copy_centre_cb(GtkAction * a, VikWindow * vw)
{
	...
	coord = vw->window->viewport->get_center();
	vik_coord_to_utm(coord, &utm);
...
static void preferences_change_update(VikWindow * vw, void * data)
{
	...
	std::list<Layer *> * layers = vw->window->layers_panel->get_all_layers_of_type(VIK_LAYER_TRW, true);
	if (!layers || layers->empty()) {
		return;
	}
...
static void preferences_cb(GtkAction * a, VikWindow * vw)
{
	...
	toolbar_apply_settings(vw->window->viking_vtb, vw->main_vbox, vw->menu_hbox, true);
}
...
static void default_location_cb(GtkAction * a, VikWindow * vw)
{
	...
	/* Get current center */
	struct LatLon ll;
	vik_coord_to_latlon(vw->window->viewport->get_center(), &ll);
...
{
	double xmpp = vw->window->viewport->get_xmpp(), ympp = vw->window->viewport->get_ympp();
	if (a_dialog_custom_zoom(GTK_WINDOW(vw), &xmpp, &ympp)) {
		vw->window->viewport->set_xmpp(xmpp);
		vw->window->viewport->set_ympp(ympp);
		vw->window->draw_update();
	}
}
...
static void save_image_file(VikWindow * vw, char const *fn, unsigned int w, unsigned int h, double zoom, bool save_as_png, bool save_kmz)
{
	...
	// Try harder...
	vik_statusbar_set_message(vw->window->viking_vs, VIK_STATUSBAR_INFO, _("Generating image file..."));
	while (gtk_events_pending()) {
		gtk_main_iteration();
	}
	...
	old_xmpp = vw->window->viewport->get_xmpp();
	old_ympp = vw->window->viewport->get_ympp();
	vw->window->viewport->set_zoom(zoom);

	/* reset width and height: */
	vw->window->viewport->configure_manually(w, h);

	/* draw all layers */
	vw->window->draw_redraw();

	/* save buffer as file. */
	pixbuf_to_save = gdk_pixbuf_get_from_drawable(NULL, GDK_DRAWABLE(vw->window->viewport->get_pixmap()), NULL, 0, 0, 0, 0, w, h);
	if (!pixbuf_to_save) {
		...
		// goto cleanup;
		vik_statusbar_set_message(vw->window->viking_vs, VIK_STATUSBAR_INFO, "");
		...
		vw->window->viewport->set_xmpp(old_xmpp);
		vw->window->viewport->set_ympp(old_ympp);
		vw->window->viewport->configure();
		vw->window->draw_update();
	...
	if (save_kmz) {
		...
		vw->window->viewport->get_min_max_lat_lon(&south, &north, &west, &east);
...
void Window::save_image_dir(char const * fn, unsigned int w, unsigned int h, double zoom, bool save_as_png, unsigned int tiles_w, unsigned int tiles_h)
{
	...
	assert (vw->window->viewport->get_coord_mode() == VIK_COORD_UTM);

	if (g_mkdir(fn,0777) != 0) {
		fprintf(stderr, "WARNING: %s: Failed to create directory %s\n", __FUNCTION__, fn);
	}

	utm_orig = *((const struct UTM *) vw->window->viewport->get_center());
	...
	for (y = 1; y <= tiles_h; y++) {
		for (x = 1; x <= tiles_w; x++) {
			...
			vw->window->viewport->set_center_utm(&utm, false);
	...
	vw->window->viewport->set_center_utm(&utm_orig, false);
	vw->window->viewport->set_xmpp(old_xmpp);
	vw->window->viewport->set_ympp(old_ympp);
	vw->window->viewport->configure();
	vw->window->draw_update();
...
static void draw_to_image_file_current_window_cb(GtkWidget* widget,GdkEventButton *event,void * *pass_along)
{
	...
	width = vw->window->viewport->get_width() * vw->window->viewport->get_xmpp() / zoom;
	height = vw->window->viewport->get_height() * vw->window->viewport->get_xmpp() / zoom;
...
static char * draw_image_filename(VikWindow * vw, img_generation_t img_gen)
{
	...
	if (img_gen != VW_GEN_DIRECTORY_OF_IMAGES) {
		...
		if (img_gen == VW_GEN_KMZ_FILE) {
			...
			if (!vw->window->draw_image_save_as_png) {
				gtk_file_chooser_set_filter(chooser, filter);
			}
		...
		if (vw->window->viewport->get_coord_mode() != VIK_COORD_UTM) {
...
void Window::draw_to_image_file(img_generation_t img_gen)
{
	...
	width_spin = gtk_spin_button_new(GTK_ADJUSTMENT(gtk_adjustment_new(vw->window->draw_image_width, 10, 50000, 10, 100, 0)), 10, 0);
	height_label = gtk_label_new(_("Height (pixels):"));
	height_spin = gtk_spin_button_new(GTK_ADJUSTMENT(gtk_adjustment_new(vw->window->draw_image_height, 10, 50000, 10, 100, 0)), 10, 0);
#ifdef WINDOWS
	...
	double mpp = vw->window->viewport->get_xmpp();
	...
	if (gtk_dialog_run(GTK_DIALOG(dialog)) == GTK_RESPONSE_ACCEPT) {
		...
		if (img_gen == VW_GEN_SINGLE_IMAGE) {
			save_image_file(vw, fn,
					vw->window->draw_image_width = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(width_spin)),
					vw->window->draw_image_height = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(height_spin)),
					zoom,
					vw->window->draw_image_save_as_png = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(png_radio)),
					false);
		} else if (img_gen == VW_GEN_KMZ_FILE) {
			// Remove some viewport overlays as these aren't useful in KMZ file.
			bool restore_xhair = vw->window->viewport->get_draw_centermark();
			if (restore_xhair) {
				vw->window->viewport->set_draw_centermark(false);
			}
			bool restore_scale = vw->window->viewport->get_draw_scale();
			if (restore_scale) {
				vw->window->viewport->set_draw_scale(false);
			}
			...
			if (restore_xhair) {
				vw->window->viewport->set_draw_centermark(true);
			}

			if (restore_scale) {
				vw->window->viewport->set_draw_scale(true);
			}
...
static void draw_to_kmz_file_cb(GtkAction * a, VikWindow * vw)
{
	...
	vw->window->draw_to_image_file(VW_GEN_KMZ_FILE);
}

static void draw_to_image_file_cb(GtkAction * a, VikWindow * vw)
{
	vw->window->draw_to_image_file(VW_GEN_SINGLE_IMAGE);
}

static void draw_to_image_dir_cb(GtkAction * a, VikWindow * vw)
{
	vw->window->draw_to_image_file(VW_GEN_DIRECTORY_OF_IMAGES);
}
...
static void window_change_coord_mode_cb(GtkAction * old_a, GtkAction * a, VikWindow * vw)
{
	...
	if (!vw->window->only_updating_coord_mode_ui) {
		VikViewportDrawMode olddrawmode = vw->window->viewport->get_drawmode();
		if (olddrawmode != drawmode) {
			/* this takes care of coord mode too */
			vw->window->viewport->set_drawmode(drawmode);
			if (drawmode == VIK_VIEWPORT_DRAWMODE_UTM) {
				vw->window->layers_panel->change_coord_mode(VIK_COORD_UTM);
			} else if (olddrawmode == VIK_VIEWPORT_DRAWMODE_UTM) {
				vw->window->layers_panel->change_coord_mode(VIK_COORD_LATLON);
			}
			vw->window->draw_update();
		}
	}
...
static void set_bg_color(GtkAction * a, VikWindow * vw)
{
	...
	GdkColor * color = vw->window->viewport->get_background_gdkcolor();
	...
	if (gtk_dialog_run(GTK_DIALOG(colorsd)) == GTK_RESPONSE_OK) {
		...
		vw->window->viewport->set_background_gdkcolor(color);
		vw->window->draw_update();
	}
	free(color);
...
static void set_highlight_color(GtkAction * a, VikWindow * vw)
{
	...
	GdkColor * color = vw->window->viewport->get_highlight_gdkcolor();
	gtk_color_selection_set_previous_color(GTK_COLOR_SELECTION(gtk_color_selection_dialog_get_color_selection(GTK_COLOR_SELECTION_DIALOG(colorsd))), color);
	gtk_color_selection_set_current_color(GTK_COLOR_SELECTION(gtk_color_selection_dialog_get_color_selection(GTK_COLOR_SELECTION_DIALOG(colorsd))), color);
	if (gtk_dialog_run(GTK_DIALOG(colorsd)) == GTK_RESPONSE_OK) {
		gtk_color_selection_get_current_color(GTK_COLOR_SELECTION(gtk_color_selection_dialog_get_color_selection(GTK_COLOR_SELECTION_DIALOG(colorsd))), color);
		vw->window->viewport->set_highlight_gdkcolor(color);
		vw->window->draw_update();
	}
	free(color);
	gtk_widget_destroy(colorsd);
...
static void window_create_ui(VikWindow * window)
{
	...
	toolbar_action_tool_entry_register(window->window->viking_vtb, &pan_tool.radioActionEntry);
	toolbar_action_tool_entry_register(window->window->viking_vtb, &zoom_tool.radioActionEntry);
	toolbar_action_tool_entry_register(window->window->viking_vtb, &ruler_tool.radioActionEntry);
	toolbar_action_tool_entry_register(window->window->viking_vtb, &select_tool.radioActionEntry);
	...
	gtk_action_group_add_toggle_actions(action_group, toggle_entries, G_N_ELEMENTS (toggle_entries), window->window);
	...
	for (i=0; i < G_N_ELEMENTS (entries); i++) {
		if (entries[i].callback) {
			toolbar_action_entry_register(window->window->viking_vtb, &entries[i]);
		}
	}
	...
	for (i=0; i < G_N_ELEMENTS (toggle_entries); i++) {
		if (toggle_entries_toolbar_cb[i]) {
			toolbar_action_toggle_entry_register(window->window->viking_vtb, &toggle_entries[i], (void *) toggle_entries_toolbar_cb[i]);
		}
	}

	for (i=0; i < G_N_ELEMENTS (mode_entries); i++) {
		toolbar_action_mode_entry_register(window->window->viking_vtb, &mode_entries[i]);
	}
	...
	for (i=0; i<VIK_LAYER_NUM_TYPES; i++) {
		...
		for (j = 0; j < vik_layer_get_interface((VikLayerTypeEnum) i)->tools_count; j++) {
			...
			toolbar_action_tool_entry_register(window->window->viking_vtb, &(vik_layer_get_interface((VikLayerTypeEnum) i)->tools[j].radioActionEntry));
	...
	window->window->setup_recent_files();
}
