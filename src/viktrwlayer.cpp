...
bool LayerTRW::find_by_date(char const * date_str, VikCoord * position, VikViewport * vvp, bool do_tracks, bool select)
{
	...
	if (select && df.found) {
		if (do_tracks && df.trk) {
			...
			vik_treeview_select_iter(VIK_LAYER((VikTrwLayer *) this->vtl)->vt, waypoints_iters.at(df.wp_uid), true);
		}
		vik_layer_emit_update(VIK_LAYER((VikTrwLayer *) this->vtl));
	}
	return df.found;
}
...
static bool trw_layer_paste_item ( VikTrwLayer *vtl, int subtype, uint8_t *item, size_t len )
{
  ...
  if ( subtype == VIK_TRW_LAYER_SUBLAYER_WAYPOINT )
  {
    ...
    waypoint_convert (NULL, wp, &vtl->trw.coord_mode);
    free( name );
...
void LayerTRW::draw_with_highlight(Viewport * viewport, bool highlight)
{
	...
	init_drawing_params(&dp, (VikTrwLayer *) this->vtl, viewport, highlight);
...
{
	VikLayer * parent = VIK_LAYER((VikTrwLayer *) this->vtl);
...
void LayerTRW::draw_highlight_item(Track * trk, Waypoint * wp, Viewport * viewport)
{
	...
	static DrawingParams dp;
	init_drawing_params(&dp, (VikTrwLayer *) this->vtl, viewport, true);
...
{
	vik_treeview_add_sublayer(vt, layer_iter, &(track_iter), _("Tracks"), (VikTrwLayer *) this->vtl, NULL, VIK_TRW_LAYER_SUBLAYER_TRACKS, NULL, false, 0);
}
...
{
	vik_treeview_add_sublayer(vt, layer_iter, &(waypoint_iter), _("Waypoints"), (VikTrwLayer *) this->vtl, NULL, VIK_TRW_LAYER_SUBLAYER_WAYPOINTS, NULL, false, 0 );
}
...
{
	vik_treeview_add_sublayer(vt, layer_iter, &(route_iter), _("Routes"), (VikTrwLayer *) this->vtl, NULL, VIK_TRW_LAYER_SUBLAYER_ROUTES, NULL, false, 0);
}
 ...
 */
void LayerTRW::set_statusbar_msg_info_trkpt(Trackpoint * tp) // kamilHERE
{
	...
	vik_statusbar_set_message(vik_window_get_statusbar(VIK_WINDOW(VIK_GTK_WINDOW_FROM_LAYER((VikTrwLayer *) this->vtl))), VIK_STATUSBAR_INFO, msg);
	free(msg);
...
{
	VikTrwLayer * vtl = (VikTrwLayer *) this->vtl;
...
static void trw_layer_auto_view ( menu_array_layer values )
{
  ...
  if (vtl->trw.auto_set_view(vik_layers_panel_get_viewport (vlp) ) ) {
...
{
	char * default_name = highest_wp_number_get((VikTrwLayer *) this->vtl);
	...
	returned_name = a_dialog_waypoint(w, default_name, (VikTrwLayer *) this->vtl, wp, this->coord_mode, true, &updated);
...
static void trw_layer_new_wp ( menu_array_layer values )
{
  ...
  if (vtl->trw.new_waypoint(VIK_GTK_WINDOW_FROM_LAYER(vtl), vik_layers_panel_get_viewport(vlp)->port.get_center()) ) {
    trw_layer_calculate_bounds_waypoints ( vtl );
...
static void trw_layer_auto_routes_view ( menu_array_layer values )
{
  ...
  if (vtl->trw.routes.size() > 0 ) {
    ...
    vtl->trw.zoom_to_show_latlons(vik_layers_panel_get_viewport (vlp), maxmin);
    vik_layers_panel_emit_update ( vlp );
  }
}
...
void LayerTRW::add_waypoint(Waypoint * wp, char const * name)
{
	...
	if (parent->realized) {
		...
		if (waypoints.size() == 0) {
			((VikTrwLayer *) this->vtl)->trw.add_sublayer_waypoints(parent->vt, &(parent->iter));
		}
...
void LayerTRW::add_track(Track * trk, char const * name)
{
	...
	if (parent->realized) {
		// Do we need to create the sublayer:
		if (tracks.size() == 0) {
			((VikTrwLayer *) this->vtl)->trw.add_sublayer_tracks(parent->vt, &(parent->iter));
		}
...
void LayerTRW::add_route(Track * trk, char const * name)
{
	...
	if (parent->realized) {
		// Do we need to create the sublayer:
		if (routes.size() == 0) {
			((VikTrwLayer *) this->vtl)->trw.add_sublayer_routes(parent->vt, &(parent->iter));
		}
...
{
	VikTrwLayer * vtl_src = (VikTrwLayer *) this->vtl;
 ...
 */
void LayerTRW::update_treeview(Track * trk) /* kamilHERE */
{
	sg_uid_t uid = 0;
	if (trk->is_route) {
...
static void trw_layer_split_at_selected_trackpoint(VikTrwLayer * vtl, int subtype)
{
	...
	if (vtl->trw.current_tpl->next && vtl->trw.current_tpl->prev) {
		char * name = vtl->trw.new_unique_sublayer_name(subtype, vtl->trw.current_tp_track->name);
		if (name) {
...
static void vik_trw_layer_uniquify_tracks ( VikTrwLayer *vtl, VikLayersPanel *vlp, std::unordered_map<sg_uid_t, Track *> & track_table, bool ontrack )
{
  ...
  while ( udata.has_same_track_name ) {
    ...
    Track * trk;
    if ( ontrack )
      trk = vtl->trw.get_track(udata.same_track_name);
    else
      trk = vtl->trw.get_route(udata.same_track_name);
    ...
    // Rename it
    char *newname = vtl->trw.new_unique_sublayer_name(VIK_TRW_LAYER_SUBLAYER_TRACK, udata.same_track_name);
...
static void vik_trw_layer_uniquify_waypoints ( VikTrwLayer *vtl, VikLayersPanel *vlp )
{
  ...
  while ( udata.has_same_waypoint_name ) {
    ...
    Waypoint * wp = vtl->trw.get_waypoint((const char *) udata.same_waypoint_name);
		...
		// Rename it
		char *newname = vtl->trw.new_unique_sublayer_name(VIK_TRW_LAYER_SUBLAYER_WAYPOINT, udata.same_waypoint_name);

		vtl->trw.waypoint_rename(wp, newname);
...
static void trw_layer_insert_tp_beside_current_tp ( VikTrwLayer *vtl, bool before )
{
  ...
  if ( tp_other ) {
    ...
    vik_coord_load_from_latlon ( &(tp_new->coord), vtl->trw.coord_mode, &ll_new );
...
static void my_tpwin_set_tp ( VikTrwLayer *vtl )
{
  ...
  vik_coord_load_from_latlon ( &vc, vtl->trw.coord_mode, &center );
...
static void highest_wp_number_remove_wp(VikTrwLayer *vtl, const char *old_wp_name)
{
  ...
  if ( vtl->highest_wp_number == old_wp_num ) {
    ...
    while ( vtl->highest_wp_number > 0 && ! vtl->trw.get_waypoint((const char *) buf)) {
      vtl->highest_wp_number--;
      snprintf(buf,4,"%03d", vtl->highest_wp_number );
    }
  }
