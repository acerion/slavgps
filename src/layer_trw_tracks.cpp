...
void LayerTRWTracks::find_maxmin_in_tracks(struct LatLon maxmin[2])
{
	for (auto i = this->tracks.begin(); i != this->tracks.end(); i++) {
...
#if 0
Track * LayerTRWTracks::find_track_by_name(char const * name)
{
	for (auto i = this->tracks.begin(); i != this->tracks.end(); i++) {
...
void LayerTRWTracks::calculate_bounds_tracks(void)
{
	for (auto i = this->tracks.begin(); i != this->tracks.end(); i++) {
