...
static void gpx_write_trackpoint(Trackpoint * tp, GPXWriteContext * context)
{
	...
	if (tp->altitude.is_valid()) {
		...
		s_alt = Altitude(0.0, AltitudeType::Unit::E::Metres).value_to_string_for_file();
	}
	if (!s_alt.isEmpty()) {
...
SaveStatus GPX::write_layer_to_file(FILE * file, LayerTRW * trw, GPXWriteOptions * options)
{
	...
	if (trw->get_waypoints_visibility() || (options && options->hidden)) {
		std::list<Waypoint *> waypoints = trw->waypoints_node().children_list();
