...
VikTrackpoint *vik_track_get_closest_tp_by_percentage_time ( VikTrack *tr, double reltime, time_t *seconds_from_start )
{
  if ( !tr->trackpoints )
    return NULL;
...
#define vtm_append(s) \
  ...
  ntp = 0;
  while (tps) {
    g_byte_array_append(b, (uint8_t *)tps->data, sizeof(VikTrackpoint));
