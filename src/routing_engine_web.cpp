...
QString RoutingEngineWeb::get_url_for_track(Track * trk)
{
	...
	ctx.url_parts[1] = substitute_latlon(this->url_start_ll_fmt, tp->coord.get_latlon()); /* TODO_LATER: have more control and checks for list index. */

	tp = *std::prev(trk->trackpoints.end());
	ctx.url_parts[len - 2] = substitute_latlon(this->url_stop_ll_fmt, tp->coord.get_latlon()); /* TODO_LATER: have more control and checks for list index. */
